...
export const securityMiddleware = async (req, res, next) => {
    try {
        ...
        if(decision.isDenied() && decision.reason.isShield()){
            logger.warn('Sheild request blocked', { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path, method: req.method });
            return res.status(403).json({ error: 'Forbidden', message: 'Request blocked by security policy' });
        }
        if(decision.isDenied() && decision.reason.isRateLimit()){
            logger.warn('Rate Limit exceeded', { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path });
            return res.status(403).json({ error: 'Forbidden', message: 'Automatted requests are not allowed' });
        }
